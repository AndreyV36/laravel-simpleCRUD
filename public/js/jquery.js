...
(function (window, undefined) {
  ...
  jQuery.support = function () {
    ...
    div.innerHTML = ""; // Check if div with explicit width and no margin-right incorrectly
    ...
    fragment = select = opt = marginDiv = div = input = null; // Run tests that need a body at doc ready
    ...
    jQuery(function () {
      ...
      support.reliableHiddenOffsets = isSupported && tds[0].offsetHeight === 0; // Figure out if the W3C box model works as expected
      ...
      if (typeof div.style.zoom !== "undefined") {
        ...
        div.style.zoom = 1;
        support.inlineBlockNeedsLayout = div.offsetWidth === 2; // Check if elements with layout shrink-wrap their children
        // (IE 6 does this)
  ...
  jQuery.fn.extend({
    data: function data(key, value) {
      ...
      if (value === undefined) {
        data = this.triggerHandler("getData" + parts[1] + "!", [parts[0]]); // Try to fetch any internally stored data first
    ...
    removeAttr: function removeAttr(elem, value) {
      ...
      if (value && elem.nodeType === 1) {
        ...
        for (; i < l; i++) {
          ...
          if (name) {
            propName = jQuery.propFix[name] || name; // See #9699 for explanation of this approach (setting first, then removal)
  ...
  jQuery.event = {
    ...
    dispatch: function dispatch(event) {
      ...
      args[0] = event;
      event.delegateTarget = this; // Determine handlers that should run if there are delegated events
  ...
  jQuery.fn.extend({
    text: function text(_text) {
      ...
      if (_typeof(_text) !== "object" && _text !== undefined) {
    ...
    html: function html(value) {
      if (value === undefined) {
        return this[0] && this[0].nodeType === 1 ? this[0].innerHTML.replace(rinlinejQuery, "") : null; // See if we can take a shortcut and just use innerHTML
        ...
        if (typeof elem === "string") {
          if (!rhtml.test(elem)) {
            ...
            var tag = (rtagName.exec(elem) || ["", ""])[1].toLowerCase(),
                ...
                depth = wrap[0],
                div = context.createElement("div"); // Append wrapper element to unknown element safe doc fragment
    ...
    if (jQuery.isArray(obj)) {
      ...
      jQuery.each(obj, function (i, v) {
        if (traditional || rbracket.test(prefix)) {
          ...
          buildParams(prefix + "[" + (_typeof(v) === "object" || jQuery.isArray(v) ? i : "") + "]", v, traditional, add);
        }
      });
    } else if (!traditional && obj != null && _typeof(obj) === "object") {
              ...
              try {
                ...
                if (_callback && (isAbort || xhr.readyState === 4)) {
                  ...
                  if (isAbort) {
                    ...
                    responses.text = xhr.responseText; // Firefox throws an exception when accessing
  ...
  jQuery.extend({
    speed: function speed(_speed, easing, fn) {
      var opt = _speed && _typeof(_speed) === "object" ? jQuery.extend({}, _speed) : {
  ...
  jQuery.each(["Height", "Width"], function (i, name) {
    var type = name.toLowerCase(); // innerHeight and innerWidth
    ...
    jQuery.fn[type] = function (size) {
      ...
      if (jQuery.isWindow(elem)) {
        ...
        return elem.document.compatMode === "CSS1Compat" && docElemProp || body && body["client" + name] || docElemProp; // Get document width or height
        ...
        var orig = jQuery.css(elem, type),
            ret = parseFloat(orig);
        return jQuery.isNumeric(ret) ? ret : orig; // Set the width or height on the element (default to pixels if value is unitless)
      } else {
...
module.exports = __webpack_require__(/*! /var/www/laravel/resources/js/jquery.js */"./resources/js/jquery.js");
